...
// Definição de tipo específico para o formulário de endereço, onde 'numero' é string
      ...
      buscarClientePorId(clienteId)
        .then((data: ClienteResponseDTO) => {
          setClienteData({
            ...
            sobrenome: data.sobrenome,
            dataNascimento:
              data.dataNascimento && data.dataNascimento.includes('/')
                ? data.dataNascimento.split('/').reverse().join('-')
                : data.dataNascimento || '',
            documento: (data.documento || '').replace(/\D/g, ''),
          });
...
<<<<<<< HEAD
          if (data.contatos && data.contatos.length > 0 && data.contatos[0]) {
            ...
            setContatoData({
              ...
              tipoContato: contatoPrincipal.tipoContato,
            });
            setCurrentContatoId(contatoPrincipal.idContato);
          }
=======
...
>>>>>>> dd583459bef31fabd0d1b8b4b8eaf4c633191e84
          ...
          if (data.enderecos && data.enderecos.length > 0 && data.enderecos[0]) {
            ...
            setEnderecoData({
              cep: (endPrincipal.cep || '').replace(/\D/g, ''),
              numero: `${endPrincipal.numero || ''}`.replace(/\D/g, ''),
              logradouro: endPrincipal.logradouro || '',
              bairro: endPrincipal.bairro || '',
              localidade: endPrincipal.localidade || '',
              uf: endPrincipal.uf || '',
              complemento: endPrincipal.complemento || '',
              latitude: endPrincipal.latitude || 0,
              longitude: endPrincipal.longitude || 0,
            });
            setCurrentEnderecoId(endPrincipal.idEndereco);
        ...
        .catch((error) => {
          ...
          setErro(`Falha ao carregar dados do cliente: ${errorMessage}`);
        })
        .finally(() => setInitialLoading(false));
    } else {
      setErro('ID do cliente inválido ou não fornecido.');
...
<<<<<<< HEAD
  const handleCepBlur = async () => {
    ...
    if (cepLimpo.length === 8) {
      ...
      try {
        ...
        setMensagem(
          'Dados do CEP carregados. Preencha/verifique o número e clique em "Obter Coordenadas" se necessário.'
        );
      } catch (error: unknown) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        setErro(`Falha na busca do endereço: ${errorMessage}. Preencha manualmente.`);
        setMensagem('');
      } finally {
        setBuscandoCep(false);
      }
  ...
  const handleGerarCoordenadasClick = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      setErro(`Falha ao gerar coordenadas: ${errorMessage}`);
      setMensagem('');
    } finally {
...
<<<<<<< HEAD
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      const apiErrorMessage = errorMessage || 'Ocorreu uma falha desconhecida.';
      setErro(
        apiErrorMessage.startsWith('Falha ao atualizar cliente:')
          ? apiErrorMessage
          : `Falha ao atualizar cliente: ${apiErrorMessage}`
      );
      setMensagem('');
    } finally {
...
>>>>>>> dd583459bef31fabd0d1b8b4b8eaf4c633191e84
  ...
  return (
    <div className="container">
      ...
      <form onSubmit={handleSubmit} className="form-container" autoComplete="off">
          ...
          <div className="form-row">
            ...
            <div className="form-group flex-item">
              ...
              <input
                ...
                onChange={handleClienteChange}
                maxLength={14}
                placeholder="Só números CPF/CNPJ"
              />
            </div>
          </div>
        </fieldset>
        ...
        <fieldset className="form-section">
          <legend className="section-title">
            📞 Contato Principal (ID: {currentContatoId || 'N/A'})
          </legend>
          <div className="form-row">
            <div className="form-group basis-ddd">
              <label htmlFor="alt-ddd">DDD:</label>
        ...
        <fieldset className="form-section">
          <legend className="section-title">
            🏠 Endereço Principal (ID: {currentEnderecoId || 'N/A'})
          </legend>
          <div className="form-row">
            <div className="form-group basis-cep">
              <label htmlFor="alt-cep">📍 CEP:</label>
        ...
        <button
          type="submit"
          disabled={buscandoCep || buscandoCoords || loadingSubmit || initialLoading}
      ...
      {mensagem && !erro && (
        <p className="message success" style={{ marginTop: '15px' }}>
          {mensagem}
        </p>
      )}
      {erro && (
