...
// Apenas o tipo que está sendo usado é mantido.
type EnderecoFormData = Omit<EnderecoRequestDTO, 'numero'> & {
  numero: string;
};

// A linha do 'LocalEnderecoState' foi removida daqui.
...
export default function CadastrarClientePage() {
  ...
  // Funções de manipulação de mudança nos inputs
  const handleClienteChange = (e: ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    if (name === 'documento') {
      const cleanedValue = value.replace(/\D/g, '');
      setClienteData((prev) => ({ ...prev, [name]: cleanedValue.slice(0, 14) }));
    } else {
  ...
  const handleEnderecoChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'cep' || name === 'numero') {
      ...
      setEnderecoData((prev) => ({ ...prev, [name]: isNaN(parsed) ? 0 : parsed }));
    } else if (name === 'uf') {
  ...
  const handleCepBlur = async () => {
    ...
    if (cepLimpo.length === 8) {
      ...
      try {
        ...
        setMensagem('Dados do CEP carregados. Preencha o número e clique em "Obter Coordenadas".');
      } catch (error: unknown) {
  ...
  const handleGerarCoordenadasClick = async () => {
    ...
    try {
      ...
      if (!coordenadas.latitude || !coordenadas.longitude) {
        setErro('Não foi possível obter coordenadas. Verifique os dados ou preencha manualmente.');
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (cleanedDocumento.length < 11 || cleanedDocumento.length > 14) { setErro('Documento (CPF/CNPJ) deve ter 11 ou 14 números.'); documentoRef.current?.focus(); return; }
    ...
    if (!cleanedDdd || cleanedDdd.length < 2 || cleanedDdd.length > 3) { setErro('DDD do contato deve ter entre 2 e 3 dígitos.'); dddRef.current?.focus(); return; }
    if (!cleanedTelefone || cleanedTelefone.length < 8 || cleanedTelefone.length > 9) { setErro('Telefone principal deve ter entre 8 e 9 dígitos.'); telefoneRef.current?.focus(); return; }
    ...
    const cleanedNumero = parseInt(enderecoData.numero.replace(/\D/g, ''), 10) || 0;
    if (!cleanedCep || cleanedCep.length !== 8) { setErro('CEP do endereço deve ter 8 dígitos.'); cepRef.current?.focus(); return; }
    ...
    if ((enderecoData.latitude || 0) === 0 || (enderecoData.longitude || 0) === 0) { setErro("Latitude e Longitude são obrigatórias. Use 'Obter Coordenadas' ou preencha."); latitudeRef.current?.focus(); return; }
    // --- Fim das Validações ---
    ...
    try {
      ...
      const enderecoPayload: EnderecoRequestDTO = {
        ...
        latitude: enderecoData.latitude || 0,
        longitude: enderecoData.longitude || 0,
      };
      ...
      // 3. Criar Cliente e associar IDs
      setMensagem('Criando cliente...');
      ...
      <form onSubmit={handleSubmit} className="form-container" autoComplete="off">
        ...
        <fieldset className="form-section coordinate-section" style={{ textAlign: 'center' }}>
          ...
          <p style={{ marginBottom: '1rem', fontSize: '0.9em' }}>
            Após preencher o endereço, clique abaixo para obter as coordenadas.
            <br />Ou preencha/ajuste manualmente.
          </p>
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            {buscandoCoords && <p className="message info" style={{ marginTop: '0.5rem' }}>Consultando serviço...</p>}
          </div>
          <div className="form-row">
              <div className="form-group flex-item">
                  <label htmlFor="latitude" style={{ display: 'block', marginBottom: '0.3rem', fontWeight: '500' }}>Latitude:</label>
                  <input id="latitude" ref={latitudeRef} type="number" step="any" name="latitude" value={enderecoData.latitude === 0 ? '' : String(enderecoData.latitude)} onChange={handleEnderecoChange} placeholder="Ex: -23.550520" style={{ textAlign: 'center' }}/>
              </div>
              <div className="form-group flex-item">
                  <label htmlFor="longitude" style={{ display: 'block', marginBottom: '0.3rem', fontWeight: '500' }}>Longitude:</label>
                  <input id="longitude" ref={longitudeRef} type="number" step="any" name="longitude" value={enderecoData.longitude === 0 ? '' : String(enderecoData.longitude)} onChange={handleEnderecoChange} placeholder="Ex: -46.633308" style={{ textAlign: 'center' }}/>
              </div>
          </div>
          {(enderecoData.latitude !== 0 || enderecoData.longitude !== 0) && !buscandoCoords && (
            <div className="coordinates-display" style={{ marginTop: '1rem', padding: '10px', backgroundColor: '#e9f5e9', borderRadius: '4px', border: '1px solid #c8e6c9' }}>
              <p style={{ margin: 0, fontWeight: 'bold', color: '#1b5e20' }}>
                ...
                Lon: <span style={{ fontFamily: 'monospace' }}>{enderecoData.longitude.toFixed(7)}</span>
              </p>
            </div>
          )}
        </fieldset>
        ...
        >
          {loadingSubmit ? 'Salvando...' : (buscandoCep || buscandoCoords ? 'Aguarde...' : 'Salvar Usuário Completo')}
        </button>
      </form>
