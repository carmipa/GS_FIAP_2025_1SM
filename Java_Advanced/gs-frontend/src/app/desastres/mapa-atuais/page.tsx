...
import type { EventMapMarkerData } from '@/components/EonetEventMap'; // Assumindo que esta interface está em EonetEventMap
...
export default function MapaEventosAtuaisNasaPage() {
    ...
    useEffect(() => {
        const fetchInitialMostRecentEvent = async () => {
            setLoading(true);
            setError(null);
            setInfoMessage(`Buscando o evento global mais recente (últimos ${fetchDaysGlobalRecente} dias, status: ${fetchStatusGlobalRecente || 'todos'})...`);
            setMarkers([]);
    ...
    return (
        <div className="container_mapa_eventos_atuais_page" style={{paddingBottom: '20px'}}>
            ...
            <p style={{textAlign: 'center', marginBottom: '20px', fontSize: '0.9em', color: '#555'}}>
                Busca o evento global mais recente (últimos {fetchDaysGlobalRecente} dias, status: {fetchStatusGlobalRecente || 'todos'}) ao carregar, ou use os filtros de data abaixo.
            </p>
            ...
            {/* Seção de Detalhes do Evento (se um único evento estiver em foco) */}
