...
export default function MapaEventosAtuaisNasaPage() {
  ...
  const queryLimitDateRange = 100;
  const queryStatusDateRange = 'all';

  // Busca inicial: evento global mais recente
  useEffect(() => {
  ...
  // Paginação
  const totalEvents = detailedEventsList.length;
  const totalPages = Math.ceil(totalEvents / pageSize);
  const paginatedEvents = detailedEventsList.slice((currentPage - 1) * pageSize, currentPage * pageSize);
  ...
  return (
    <div className="container_mapa_eventos_atuais_page" style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      {/* PAGINAÇÃO E SELETOR CENTRALIZADOS */}
      {detailedEventsList.length > 0 && (
        <div
          style={{
            display: 'flex',
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <label htmlFor="pageSize" style={{ marginRight: 2 }}>Exibir:</label>
            <select
              id="pageSize"
              value={pageSize}
      ...
      {paginatedEvents.length > 0 && (
        <section className="multiple-events-list" style={{ margin: '30px auto', maxWidth: '950px' }}>
          <h3 style={{ textAlign: 'center', fontSize: '1.6em', color: '#333', borderBottom: '2px solid #007bff', paddingBottom: '10px', marginBottom: '25px' }}>
            {totalEvents} Evento(s) Encontrado(s) no Período
          </h3>
      ...
      {/* Mensagens de status */}
      {loading && !paginatedEvents.length && !displayedEventDetails && (
      ...
      {infoMessage && !loading && !error && paginatedEvents.length === 0 && !displayedEventDetails && (
        <p className="message info" style={{ textAlign: 'center', margin: '20px 0', color: '#555', fontSize: '1em' }}>{infoMessage}</p>
      )}

      {/* Mapa principal */}
      {(!loading || markers.length > 0) && !error && paginatedEvents.length === 0 && (
