...
@Tag(name = "EONET Eventos", description = "Endpoints para gerenciamento de eventos EONET, incluindo sincronização com NASA API")
public class EonetController {
    ...
    @PostMapping
    public ResponseEntity<EonetResponseDTO> salvarEventoEonetManualmente( // Renomeado no serviço, mantendo consistência
    ...
    public ResponseEntity<EonetResponseDTO> atualizarEventoEonetManualmente( // Renomeado no serviço
    ...
    public ResponseEntity<List<EonetResponseDTO>> buscarEventosPorIntervaloDeData(
            ...
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) OffsetDateTime dataInicial, // CORRIGIDO
            ...
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) OffsetDateTime dataFinal) { // CORRIGIDO
    ...
    public ResponseEntity<List<EonetResponseDTO>> sincronizarEventosDaNasa(
            @Parameter(description = "Número máximo de eventos a serem buscados da API da NASA.")
            @RequestParam(required = false) Integer limit,
            @Parameter(description = "Número de dias anteriores para buscar eventos (ex: 10 para os últimos 10 dias).")
            @RequestParam(required = false) Integer days,
            @Parameter(description = "Status dos eventos a serem buscados (ex: 'open' ou 'closed'). Padrão 'open'.")
            @RequestParam(defaultValue = "open") String status,
            @Parameter(description = "Fonte dos eventos (ex: 'PDC', 'MRR').")
